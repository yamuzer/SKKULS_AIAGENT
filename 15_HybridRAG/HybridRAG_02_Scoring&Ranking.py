...
class GraphQueryPlan(BaseModel):
    ...
    start_entities: list[str] = Field(
        description='Graph 탐색 시작 Entity'
    )

    relation_path: list[str] = Field(
        description='순서대로 따라갈 Relation'
    )
...
2. relation_path
질문의 답을 찾기 위해 Graph에서 순서대로 따라가야 하는 Relation
...
def calculate_hybrid_score(
    ...
    retrieval_sources = document['retrieval_sources']

    graph_rank = document['graph_rank']
    vector_rank = document['vector_rank']
    distance = document['distance']

    if 'graph' in retrieval_sources:
        graph_presence_score = 2.0
    else:
        graph_presence_score = 0.0


    if 'graph' in retrieval_sources and 'vector' in retrieval_sources:
        both_source_score = 1.0
    else:
        both_source_score = 0.0
    ...
    if graph_rank is not None:
        graph_rank_score = 0.5 / graph_rank
    else:
        graph_rank_score = 0.0


    if vector_rank is not None:
        vector_rank_score = 0.75 / vector_rank
    else:
    ...
    if distance is not None:
        ...
        vector_distance_score = 0.25 * (1.0 / (1.0 + safe_distance))
    else:
    ...
    score_detail = {
        'graph_presence_score': graph_presence_score,
        'both_source_score': both_source_score,
        'graph_rank_score': graph_rank_score,
        'vector_rank_score': vector_rank_score,
        'vector_distance_score': vector_distance_score,
...
def rerank_evidence(
    ...
    for document in fused_documents:
        ...
        score_detail = calculate_hybrid_score(document)

        item['score_detail'] = score_detail
        item['hybrid_score'] = score_detail['hybrid_score']

        reranked.append(item)


    reranked.sort(
        key=lambda document: document['hybrid_score'],
        reverse=True
    )

    for index, document in enumerate(reranked, start=1):
        document['hybrid_rank']=index
...
def build_ranked_evidence_context(
        documents
):
    ...
    for document in documents:
        ...
        lines = [
            (
                f"[HYBRID RANK {document['hybrid_rank']}]"
            ),
            (
                f"문서 ID: {document['doc_id']}"
            ),
            (
                f"검색 출처: {retrieval_source_text}"
            ),
            (
                f"Hybrid Score: {document['hybrid_score']:.4f}"
            ),
            (
                f"원문: {document['text']}"
            )
...
1. Graph Context
Entity 사이의 구조와 Relation Path를 나타냅니다.

2. Ranked Evidence
Graph와 Vector 검색 결과를 통합하여 재정렬한 실제 원문입니다.


[중요 규칙]
1. 제공된 근거에서 확인 할 수 있는 사실만 사용하세요.
2. Graph Context는 관계를 파악하는데 사용하세요.
3. Ranked Evidence는 실제 사실을 확인하는데 사용하세요.
4. Hybrid Rank가 높은 문서를 우선 참고하되, 질문과 실제로 관련 있는지 확인하세요.
5. 점수가 높다는 이유만으로 질문과 관련 없는 내용을 사용하면 안됩니다.
6. GRAPH 정보와 원문이 충돌한다면 실제 원문을 우선하세요.
7. 근거가 없는 사실을 추측하지 마세요.
8. 답변 마지막에는 실제로 사용한 문서 ID를 표시하세요.
9. 근거가 부족하면 "제공된 검색 결과만으로 답할 수 없습니다."라고 답하세요
...
def hybrid_rag(
    ...
    reranked_documents = rerank_evidence(
        fused_documents
    )

    

    top_documents = select_top_evidence(
        reranked_documents,
        top_n=final_top_n
    )
    ...
    print()
    print('='*70)
    print('step 5 final Evidence Context')
    print('='*70)

    for document in top_documents:
        print()
        print(
            f"Rank {document['hybrid_rank']}"
            f"\ndoc_id: {document['doc_id']}"
            f"\n내용:\n{document['text']}"
        )


    evidence_context = build_ranked_evidence_context(top_documents)
