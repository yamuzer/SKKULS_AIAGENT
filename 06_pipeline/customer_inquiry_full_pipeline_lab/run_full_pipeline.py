...
PATHS = PipelinePaths(
    ...
    standardized_csv=(
        BASE_DIR / "output" / "standardized" / "customer_inquiry_standardized.csv"
    ),
    quality_result_csv=(
        BASE_DIR / "output" / "quality" / "customer_inquiry_quality_result.csv"
    ),
    valid_csv=BASE_DIR / "output" / "quality" / "customer_inquiry_valid.csv",
    invalid_csv=BASE_DIR / "output" / "quality" / "customer_inquiry_invalid.csv",
    ...
    quality_html=BASE_DIR / "reports" / "customer_inquiry_quality_report.html",
...
def main() -> None:
    ...
    print("=" * 80)
    print("1. 정적 고객 문의 크롤링" if not arguments.skip_crawl else "1. 크롤링 생략·기존 원천 CSV 사용")
    print("2. 기준정보 기반 표준화")
    print("3. 품질 규칙 검증")
