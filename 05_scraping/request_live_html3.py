...
all_rows = market_table.select('tr')
valid_rows = []
for row in all_rows:
    name_link = row.select_one('a[href*="/item/main.naver?code="]')
    cells = row.select(
        ':scope > td'
    )
...
cells = first_row.select(':scope > td')

for index, cell in enumerate(cells):
    print(index, cell.get_text(strip=True))
